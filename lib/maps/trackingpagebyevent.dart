...
class TrackingEvent extends KFDrawerContent {
  TrackingEvent({Key? key,required this.currentUserLocation,required this.destinationLocation, 
  ...
  final List<dynamic> destinationLocation;

  @override
  State<TrackingEvent> createState() => _TrackingEventState();
}


class _TrackingEventState extends State<TrackingEvent> {
...
// }
// void getbytes(int width, int height) async {
//   final ui.PictureRecorder pictureRecorder = ui.PictureRecorder();
//   final Canvas canvas = Canvas(pictureRecorder);
//   final Paint paint = Paint()..color = Colors.blue;
//   final Radius radius = Radius.circular(20.0);
...
// }
  ...
  void getPolyPoint() async{
    ...
    PolylineResult polylineResult = await polylinePoints.getRouteBetweenCoordinates(
      ...
      PointLatLng(widget.currentUserLocation.latitude, widget.currentUserLocation.longitude), 
      PointLatLng(widget.destinationLocation[0], widget.destinationLocation[1]));
  ...
  startPin()async{
      ...
      var bytes = request.bodyBytes;
                            ...
                            position: LatLng(widget.currentUserLocation.latitude,widget.currentUserLocation.longitude)
                            ));
                            markers.add(Marker(markerId: MarkerId("destination"),
                            position:  LatLng(widget.destinationLocation[0], widget.destinationLocation[1])
                            ));
  ...
  void FriendsMarker(){
    ...
    .then((DocumentSnapshot documentSnapshot) async {
      if (documentSnapshot.exists) {
        ...
        allfriend.addAll(documentSnapshot.get(FieldPath(['members'])));
        await FirebaseFirestore.instance.collection("users").doc(userid).update({
          "currentlocation" : [widget.currentUserLocation.latitude,widget.currentUserLocation.longitude],
          })
          .then((value){
              ...
              Column(
                ...
                children: [
                  Padding(
                    ...
                    child: Text(widget.placeName,style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700)),
                  ),
                  SizedBox(height: 10,),
                ],
              ),
                        ...
                        children:  <Widget>[
                          ...
                          GoogleMap(
                            initialCameraPosition: CameraPosition(
                              target: LatLng(widget.currentUserLocation.latitude,widget.currentUserLocation.longitude)
                              ,zoom: 14
                            ),
                          ...
                          //   icon: BitmapDescriptor.fromBytes(dataBytes.buffer!.asUint8List(), size: Size.fromHeight(3)),
                          //   // icon: BitmapDescriptor.fromBytes(dataBytes.buffer.getBytesFromCanvas()),
                          //   // icon: BitmapDescriptor.fromBytes(dataBytes!.buffer.asUint8List(),size: Size.square(0)),
                          //   position: 
                          //   LatLng(widget.currentUserLocation.latitude,widget.currentUserLocation.longitude)
                          //   ),
